...
def return_statistics(dataloader, selected_idx, logger: Optional[MyLogging] = None):
    '''
    ...
    stats = {'Selected_clean/total': r_stats[0], 'Precision': r_stats[1], 'Recall': r_stats[2], 'F1_Score': r_stats[3], 'Specificity': r_stats[4], 'Accuracy': r_stats[5]}
    if logger is not None:
        stats.update({'Selected_clean': tp})
        stats.update({'total': tp+fp})
        del stats['Selected_clean/total']
        stats['Selected_clean:total'] = round(tp/(tp+fp), 4)
        stats = {f"stats/{k}": v for k, v in stats.items()}
        logger.log(msg=stats)
    else:
        print(stats)
