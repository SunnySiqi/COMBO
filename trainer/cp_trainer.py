...
import dataloader.CP as dataloader
...
class CPTrainer(BaseTrainer):
    ...
    def __init__(self, model, metrics, optimizer, config, parse, lr_scheduler=None, len_epoch=None):
        ...
        batch_size = self.config["data_loader"]["args"]["batch_size"]  ## TODO remove hardcode

        num_workers = self.config["data_loader"]["args"]["num_workers"]
        root = config["data_loader"]["args"]["data_dir"]
        warmup_batch_size = 4 * batch_size
        self.loader = dataloader.CP_dataloader(root, batch_size, warmup_batch_size, num_workers)
