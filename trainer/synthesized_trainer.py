...
import dataloader.CIFAR as dataloader
...
class SynthesizedTrainer(BaseTrainer):
    """
    ...
    def __init__(self, model, metrics, optimizer, config, parse, lr_scheduler=None, len_epoch=None):
        ...
        noise_ratio = self.config["data_loader"]["noise"]["percent"]
        dataset = self.config["data_loader"]["args"]["dataset"]
        if self.config["data_loader"]["noise"]["type"] == "asym":
            noise_mode = "asym"
        else:
            noise_mode = "control"
        batch_size = self.config["data_loader"]["args"]["batch_size"]
        num_workers = self.config["data_loader"]["args"]["num_workers"]
        noise_file = "%s/clean_%.4f_%s_%s.npz" % (
        ...
        self.loader = dataloader.cifar_dataloader(
            dataset, noise_ratio, noise_mode, batch_size, num_workers, noise_file
        )
    ...
    def update_dataloader(
        ...
        (
            ...
            self.specificity,
            self.accuracy,
        ) = return_statistics(self.eval_loader, self.clean_idx, logger=self.logger)
    ...
    def _train_epoch_cluster_label(self, model, optimizer, epoch):
        ...
        with tqdm(self.labeled_dataloader) as progress:
            for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x) in enumerate(
                ...
                labels_u = torch.zeros(batch_size, self.cls_num).scatter_(
                    1, labels_u.view(-1, 1), 1
                )
                ...
                targets_x = labels_x / labels_x.sum(dim=1, keepdim=True)
                targets_x = targets_x.detach()

                targets_u = labels_u / labels_u.sum(dim=1, keepdim=True)
                targets_u = targets_u.detach()
                ## Unsupervised Contrastive Loss
                ...
                f_x1 = torch.nn.functional.normalize(f_x1, dim=1)
                f_x2 = torch.nn.functional.normalize(f_x2, dim=1)
                features = torch.cat([f_x1.unsqueeze(1), f_x2.unsqueeze(1)], dim=1)
    ...
    def _train_epoch(self, epoch):
        ...
        if self.detection_method == "none":
            ...
            self.purity = (
                self.labeled_dataloader.dataset.noise_label
                == self.labeled_dataloader.dataset.train_labels_gt
            ).sum() / len(self.labeled_dataloader.dataset.noise_label)
            log = {
                "loss": total_loss / num_iter,
                "learning rate": self.lr_scheduler.get_last_lr(),
                "purity:": "{} = {}/{}".format(
        ...
        for epoch in tqdm(range(self.start_epoch, self.epochs + 1), desc="Total progress: "):
            ...
            if self.mnt_mode != "off":
                ...
                if improved:
                    ...
                    # log best test_metric_overall to wandb
    ...
    def get_estimation_matrix(self, model, epoch: Optional[int] = None):
        if self.estimation_method == "growing_cluster":
            ...
            (
                ...
                all_features,
                all_indexs,
            ) = growing_cluster.cluster_stats()
            print("!!!!!TRAIN KMEANS CLUSTER GT CONFUSION MATRIX!!!!!")
            print(confusion_matrix(all_gts, all_clusterids))
            print("-------------------------------------")
            ...
            assert epoch is not None, "epoch must be specified for BLTM estimation"
            ...
            source_matrix = robot_update_t(self.trans, self.eval_loader, self.device, model, self.meta_optimizer, self.cls_num, self.loss_name, self.outer_obj)
            all_labels, all_features, all_indexs = self.get_feature(model)
            all_clusterids = None
