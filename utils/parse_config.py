...
class ConfigParser:
    ...
    def __init__(self, args, options="", timestamp=True):
        ...
        if "spence" in get_machine_name():
            if args.device:
                os.environ["CUDA_VISIBLE_DEVICES"] = args.device  ## remove this for SCC
        ...
        dataset_name = self.config["data_loader"]["args"]["dataset"]
        ...
        train_method = self.config["trainer"]["train_noise_method"]

        if args.traintools == "train_synthesized":
            noise_type = self.config["data_loader"]["noise"]["type"]
            syn_noise_percent = str(int(self.config["data_loader"]["noise"]["percent"] * 100))
            self._save_dir = (
                save_dir
                / "models"
                / dataset_name
